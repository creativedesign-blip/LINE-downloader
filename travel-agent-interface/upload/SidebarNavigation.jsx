    ...
    <div className={`${collapsed ? "workspace-collapsed p-2" : "p-3 lg:p-4"} space-y-4`}>
      ...
      <div className={`rounded-lg border bg-white ${collapsed ? "p-2" : "p-3"}`} style={{ borderColor: "#E1F5EE" }}>
        <div className={`flex ${collapsed ? "items-center justify-center" : "items-start justify-between"} gap-3`}>
          ...
          <button type="button" onClick={onToggleLineAuto} title={lineAutoTooltip} aria-label={`LINE 自動抓圖：${lineAutoAction}`} className={`flex items-center justify-center rounded-md border text-[10px] flex-shrink-0 ${collapsed ? "w-full flex-col gap-1 px-1.5 py-2 whitespace-nowrap leading-none" : "gap-1.5 px-2 py-1"}`} style={{ borderColor: lineAutoEnabled ? "#1D9E75" : "#B91C1C", color: lineAutoEnabled ? "#0F6E56" : "#991B1B" }}><Power className="w-3 h-3 flex-shrink-0" /><span>{lineAutoAction}</span></button>
        </div>
      </div>
    </div>
  );
