...
function buildTargetFromPage(summary, { id, label } = {}) {
  ...
  return {
    ...
    label: displayLabel,
    groupName,
    downloadDir: path.join('line-rpa', 'download', resolvedId, 'inbox'),
    travelDir: path.join('line-rpa', 'download', resolvedId, 'travel'),
    otherDir: path.join('line-rpa', 'download', resolvedId, 'other'),
    errorDir: path.join('line-rpa', 'download', resolvedId, 'error'),
    pageFingerprint: {
